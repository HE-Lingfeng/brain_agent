...
def resilient_get(
    ...
    *,
    ...
    """Wrap ``s.get(url)`` with retry on transport-level connection errors.

    Retries with exponential backoff: ``backoff_base ** attempt`` seconds
    (1s, 2s, 4s by default).  Does **not** refresh the session — session
    refresh is the caller's responsibility.

    Returns:
        ``(response, error_string)`` where ``error_string`` is empty on
        success, or ``"connection_error: <details>"`` on total failure.
    """
    last_error = ""
    for attempt in range(max(1, int(max_retries))):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.warning(
                    "[BRAIN wait] Connection error on GET %s (attempt %d/%d), "
                    "retrying in %.1fs: %s",
                ...
                continue
            logger.error(
                "[BRAIN wait] Connection error on GET %s after %d attempts: %s",
                url, max_retries, last_error,
            )
            return None, f"connection_error: {last_error}"
        except Exception as exc:
            last_error = f"{type(exc).__name__}: {exc}"
            logger.error("Unexpected error on GET %s: %s", url, last_error)
            return None, f"connection_error: {last_error}"
        else:
            return response, ""
...
def simulation_progress(
    ...
    retry_count = 0
    while True:
        simulation_progress_response, conn_err = resilient_get(s, simulation_progress_url)
        if conn_err:
            logger.error(
                f'Simulation {simulation_progress_url}: {conn_err}, Retry'
            )
            time.sleep(30)
            retry_count += 1
            if retry_count <= 2:
                ...
                logger.error(
                    f'Simulation {simulation_progress_url} failed after connection errors'
                )
                error_flag = True
                break
    ...
    if error_flag:
        if simulation_progress_response is not None:
...
def get_simulation_result_json(s: SingleSession, alpha_id: str) -> dict:
    ...
    if alpha_id is None:
        return {}
    while True:
        result, conn_err = resilient_get(s, brain_api_url + "/alphas/" + alpha_id)
        if conn_err:
            logger.error(f"alpha_id {alpha_id}: {conn_err}")
            return {}

...
def score_candidate(
    ...
    tags = _csv_or_list(candidate.get("failure_tags") or latest.get("failure_tags"))
    objectives = _csv_or_list(candidate.get("repair_objectives") or latest.get("repair_objectives"))
    ...
    memory = _memory_adjustment(expression, memory_context)
    ...
    breakdown = {
        ...
        "rationale": _rationale(status, tags, objectives, latest, expression, has_metrics),
    }
...
def score_candidates(
    ...
    for candidate in candidates:
        ...
        score = score_candidate(candidate, latest, memory_context)
        item = dict(candidate)
...
def _memory_adjustment(expression: str, memory_context: dict[str, Any] | None) -> dict[str, Any]:
    if not memory_context:
        return {"memory_score": 0.5, "memory_risk_penalty": 0.0, "memory_evidence": []}
    ...
    for kind, key, base_weight, stats in evidence_inputs:
        ...
        confidence = _clamp(observations / 20, 0.0, 1.0)
        ...
        hard_failures = 0
        for tag in ("syntax_error", "unknown_variable", "coverage_issue", "hard_error", "high_turnover"):
            hard_failures += int(failure_tags.get(tag, 0) or 0)
        hard_failure_rate = _clamp(hard_failures / observations, 0.0, 1.0)
        risk_penalty += hard_failure_rate * confidence * base_weight * 0.08
        evidence.append(
            {
                "kind": kind,
                "key": key,
